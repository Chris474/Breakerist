...
abstract class Skin_Base extends Elementor_Skin_Base
{
    ...
    protected function render_featured_image(array $settings)
    {
        ...
        if ($settings['use_bgimage']) {
            ...
            $this->get_parent()->set_render_attribute('featured-image', 'class', 'dce-post-bgimage');
        }
        if ($settings['use_overlay']) {
            // Use Overlay
            $this->get_parent()->set_render_attribute('featured-image', 'class', 'dce-post-overlayimage');
        }
        if ($this->get_parent()->get_settings('use_overlay_hover')) {
            // Use Overlay Hover
            $this->get_parent()->set_render_attribute('featured-image', 'class', 'dce-post-overlayhover');
        }
        $html_tag = 'div';
        if (!empty($settings['use_link'])) {
            $html_tag = 'a';
            $this->get_parent()->add_render_attribute('featured-image', 'href', $this->current_permalink);
            if (!empty($settings['open_target_blank'])) {
                $this->get_parent()->add_render_attribute('featured-image', 'target', '_blank');
            }
        }
        ...
        if (!empty($custommeta_key)) {
            ...
            switch ($metafield_type) {
                ...
                case 'image':
                    ...
                    break;
                case 'button':
                    $this->get_parent()->add_render_attribute($attribute_a_link, 'href', $meta_value);
