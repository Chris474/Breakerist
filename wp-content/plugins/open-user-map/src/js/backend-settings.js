(function(){
  ...
  jQuery(addButton).click(function(e){
    ...
    if(x < maxField){ 
        ...
        let fieldHTML = `
          <tr>
            <td>
              <input type="text" name="oum_custom_fields[${index}][label]" placeholder="Enter label" value="" />
            </td>
            <td>
              <input class="oum-switch" id="oum_custom_fields_${index}" type="checkbox" name="oum_custom_fields[${index}][required]"><label for="oum_custom_fields_${index}"></label>
            </td>
            <td>
              <input class="small-text oum-custom-field-maxlength" type="number" min="0" name="oum_custom_fields[${index}][maxlength]" />
            </td>
            <td>
        ...
        fieldHTML += `
            ...
            <td>
              <input type="text" class="oum-custom-field-options" name="oum_custom_fields[${index}][options]" placeholder="Red|Blue|Green" value="" style="visibility: hidden;" /><br><br>
              <label class="oum-custom-field-options oum-custom-field-allow-empty" style="visibility: hidden;"><input type="checkbox" name="oum_custom_fields[${index}][emptyoption]" />add empty option</label>
            </td>
            <td>
              <input type="text" name="oum_custom_fields[${index}][description]" placeholder="Enter description (optional)" value="" />
            </td>
            <td class="actions">
  ...
  function updateCustomFieldRow(el) {
    if(jQuery(el).val() == 'text' || jQuery(el).val() == 'link') {
      jQuery(el).closest('tr').find('.oum-custom-field-options').css("visibility", "hidden");
      jQuery(el).closest('tr').find('.oum-custom-field-maxlength').css("visibility", "visible");
    }else{
      jQuery(el).closest('tr').find('.oum-custom-field-options').css("visibility", "visible");
      jQuery(el).closest('tr').find('.oum-custom-field-maxlength').css("visibility", "hidden");

      if(jQuery(el).val() == 'select') {
        jQuery(el).closest('tr').find('.oum-custom-field-allow-empty').css("visibility", "visible");
      }else{
        jQuery(el).closest('tr').find('.oum-custom-field-allow-empty').css("visibility", "hidden");
      }
    }
  }
  ...
  if(jQuery('#oum_enable_user_restriction').length > 0) {
    ...
    redirectToRegistration(jQuery('#oum_enable_user_restriction').checked);
